...
\begin{itemize}
    ...
    \item \textbf{DD}: Volume difference between associated cuboids. % Diference in Dimensions
    \item \textbf{DED}: Absolute difference per axis (length, width, height). % Difference in Each Dimension 
    \item \textbf{v2v}: Minimum distance between convex hulls. % Volume-Volume Distance
    \item \textbf{IoU}: 3D intersection-over-union. % Volumetric IoU
    \item \textbf{BBD}: Dissimilarity metric for overlapping/non-overlapping boxes. % Bounding Box Disparity
\end{itemize}
