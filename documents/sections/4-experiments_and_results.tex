...
The Bounding Box Disparity (BBD) serves as a valuable metric for assessing the positional accuracy of cuboids and for readily identifying potential incorrect detections. For instance, Figure~\ref{fig:van_bbd_through_frames_a} illustrates the BBD evolution for the van throughout the scene. The horizontal line in this graph denotes the BBD metric's intrinsic threshold separating Intersection over Union (IoU) based calculations from Volume-to-Volume (V2V) based ones. An example is the initial detection of the van in frame 2, which exhibits a BBD greater than 1. This signifies that the detection does not intersect with the ground truth bounding box. 
...
Generally, a high BBD can indicate an erroneous detection, as observed in frames 12 and 13 for this object (see Figure Figure~\ref{fig:van_bbd_through_frames_c}). However, such cases may not always indicate a detection failure. They can also result from depth estimation errors. This is illustrated by the van in frame 3 (see Figure~\ref{fig:van_bbd_through_frames_b}), where the predicted depth is underestimated compared to the actual distance, yet the relative dimensions of the van are preserved.

In this sense, BBD stands out as a reliable metric for detecting potential outliers, particularly when BBD exceeds 1. On the other hand, when BBD remains below 1, it can be interpreted as an inverse confidence score: the lower the BBD, the more accurate the bounding box prediction is likely to be. Accordingly, most of the van detections maintain a BBD below 1, with an average value of approximately $0.67$ and a standar deviation of $0.11$. This suggests that, although the predicted bounding boxes may not align perfectly with the ground truth, their spatial positioning remains relatively consistent and acceptable throughout the scene.
...
It is important to note that the masks generated by the pipeline are based on different data sources and follow distinct generation processes. Specifically, the occupancy masks are obtained from the footprints of 3D bounding boxes derived from the 3D detection process, whereas the occlusion and drivable area masks are obtained from semantic segmentation masks reprojected into \aclink{BEV} space. In this context, occlusion is represented by the distorted projections of object-level semantic masks in \aclink{BEV}, which contrasts with the ray-casting approach used to generate the ground truth (see Section~\ref{sec:instance_bev_mask_methodology}). This difference in data origin and representation is crucial for interpreting the behavior observed in the evaluation metrics.
...
Figure~\ref{fig:scene_iou_occ2_masks} shows the \aclink{IoU} scores for each semantic class across all frames of the scene. The 'background' class achieves the highest \aclink{IoU} values. This behavior is expected, as a significant portion of the area classified as background corresponds to regions not visible to the front camera, which remain constant in both the ground truth and the system-generated masks. In contrast, the occupancy masks exhibit the lowest \aclink{IoU} values overall.

Although the occlusion and occupancy masks appear to be correlated, as occlusion occurs where there is occupancy, the occlusion masks consistently achieve higher \aclink{IoU} values. A common assumption is that occlusion masks are generated directly from occupancy masks, which implies that inaccuracies in occupancy would propagate to occlusion, leading to a similar degradation in performance for both. Furthermore, since occlusion depends on the presence of objects, it might be considered a secondary effect, which suggests that occupancy would likely perform at least as well, if not better. However, in this case, the occlusion masks are generated independently from the occupancy masks, relying instead on semantic segmentation projections. This independence could make occlusion masks less sensitive to errors that mainly affect occupancy masks (which often arise from the 3D detection process), potentially explaining the observed divergence in their performance and the higher \aclink{IoU} values of occlusion masks.
...
Regarding the drivable area, the behavior of the \aclink{IoU} over time reflects a direct relationship between scene complexity and pipeline performance. During the initial frames (0–10), a consistently high \aclink{IoU} is observed, which corresponds to a simple driving environment: an open road with few obstacles. However, starting from frame 11, and particularly between frames 11 and 23—a section corresponding to a curve—a progressive degradation of the \aclink{IoU} occurs. This decrease is not attributed solely to the turning maneuver itself, but rather to the fact that this curve coincides with a complex intersection, where the presence of more vehicles, pedestrians, and nearby objects generates more numerous occlusions and far more irregular contours for the drivable area.

This increased complexity exposes the pipeline's limitations on two levels. First, it amplifies the noise in the semantic segmentation output, as the model struggles to accurately delineate drivable regions when they are fragmented by multiple obstacles. Second, it highlights the inherent divergence between the model's prediction—which is based on the visual appearance of the scene—and the ground truth, which defines drivable areas using idealized, geometrically perfect polygons. The combination of these factors—high scene density and the discrepancy between a visual representation and a structured reference—explains the observed decline in performance and aligns with the results from the 3D accuracy evaluation presented in Section X.
