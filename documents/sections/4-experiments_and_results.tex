...
Regarding the drivable area, the evolution of the \aclink{IoU} over time appears to reflect a direct relationship between scene complexity and the pipeline's performance. In the initial frames (0–10), the \aclink{IoU} remains consistently high, corresponding to a relatively simple driving scenario: an open road with just a few obstacles (see Figure \hl{X}). Starting from frame 11, however, and especially between frames 11 and 23 (a segment involving a curve), a gradual decline in \aclink{IoU} is observed. This drop appears to result not only from the turning maneuver itself, but also from the increased complexity of the environment during this part of the scene. Specifically, the curve matches a busy intersection, where the presence of additional vehicles, pedestrians, and surrounding objects results in greater occlusion and more irregular, fragmented drivable area boundaries (see Figure \hl{X}).

Also, there are some particular cases at frames 30 and 31 were the driveable area clearly drops to a very low value. However, this cases are more related with a bad annotation or generation of the ground truth masks, as the vehicle footprint is clearly overestimated (see Figure \hl{X}), rather than a poor detection.

All this exposes two main limitations. First, the occupancy masks are highly correlated to the 3D detection errors exposed previously in Section~\ref{sec:dimension_estimation_accuracy}. Second, the scene's complexity amplifies the noise in the semantic segmentation output, as the model struggles to accurately delineate drivable regions when they are fragmented by multiple obstacles. However, despite this, the generated occupancy, occlusion and driveable masks are subjectively accepctable in this scene's case as it can be seen in Figure \hl{X}.
